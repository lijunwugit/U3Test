...
function delType(id) {
    ...
    $.messager.confirm('删除区域', '真的想删除吗?', function(r){
        if (r){
            $.post("/admin/delType.do",{"id":id},function(data){
                if(data.result==1){
                    $("#dg").datagrid('reload'); //刷新
                }else{
...
function DeleteByFruitName() {
    ...
    $.messager.confirm("系统提示", "你确定要删除<font color=red> " + SelectRows.length + " </font>条数据吗?", function(xo){
        if( xo ){
            $("#dg").datagrid('reload');
            $.post("/admin/delMoreType.do",{ids:SelectIndexToString},function(data){
                if( data.result>0 ){
