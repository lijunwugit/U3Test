...
<META name=GENERATOR >
    <script>
        function delHouse(obj,id) {
            $.post("/page/delHouse.do",{"id":id},function(data){
                if(data.result>0)
                {
                    //移除当前行
...
<c:forEach items="${list}" var="h">
  <TR>
    ...
    <TD class=house-type><LABEL class=ui-green><INPUT onclick="location.href='/page/showHouse.do?id=${h.id}';" value="修    改" type=button name=search></LABEL></TD>
    <TD class=house-price><LABEL class=ui-green><INPUT value="删    除" type=button  onclick="delHouse(this,${h.id})"></LABEL></TD></TR>
</c:forEach>
  </TBODY></TABLE></DIV>
<DIV class=pager>
<UL>
